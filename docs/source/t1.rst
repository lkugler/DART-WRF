...
The first block of the imports is fixed
...
The second block contains default for your settings, ``config/jet_1node.py`` and ``config/defaults.py``.
...
Now you can specify other parameters to the config (or overwrite defaults):
...
Lastly, you can set up and run the workflow:
